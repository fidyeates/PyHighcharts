		...
		for k, v in kwargs.items(): 
			...
			if k[0] in self.ALLOWED_OPTIONS:
				if isinstance(self.ALLOWED_OPTIONS[k[0]],dict):
					if len(k) > 2:	raise NotImplementedError
					else:
						if self.__validate_options__(k[1],v,self.ALLOWED_OPTIONS[k[0]][k[1]]):
							if not k[0] in self.__dict__: self.__dict__.update({k[0]:{}})
					...
					else:	raise OptionTypeError("Option Type Mismatch: Expected: %s" % self.ALLOWED_OPTIONS[k[0]])
			else:	raise OptionTypeError("Not An Accepted Option Type: %s" % k[0])
